   ...
   return plus;
	...
	public function doProcesslist() {
		...
		try {
			...
			*/
			$query = $this->_mongo->selectDB("admin")->command(['currentOp' => true, '$all' => true]);
			if ($query["inprog"]) {
				$this->progs = $query["inprog"];
			}
			//-- end:fix
			//print_r($this->progs); die();
	...
	public function doCreateDatabase() {
		if ($this->isPost()) {
			...
			$this->message = "New database created with defCol."; // fix by unixman: to create a new DB also a collection must be created inside !
			$this->_mongo->selectDb($name)->execute('function(){ return db.runCommand('.json_encode([ 'create' => 'defCol' ]).'); }');
		}
		$this->display();
	}
